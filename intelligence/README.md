...
Current acceptance status: [HACKATHON-READINESS](../docs/HACKATHON-READINESS.md).
The forecasting revision on 2026-09-23 uses corrected stockout demand and per-SKU
model selection including seasonal trend. Earlier baseline reports below describe
the original experiment; current numbers are in `data/processed/forecast_metrics.json`.
...
The API exposes saved results. `POST /recalculate` invokes `reorder.recommend` on saved forecasts and source inputs; it does not retrain, accept numerical overrides, write artifacts, or submit orders. The service holds one consistent snapshot; restart it after externally regenerating pipeline outputs. The 2026-09-23 revision feeds supported lost-demand corrections into the forecasting target and selects a method per SKU using chronological validation. A seasonal-trend candidate multiplies the prior-year matching month by the median of recent year-on-year ratios (at least three, clipped to 0.5..2.0). Selection needs four validation months, otherwise it falls back to the global winner. Model-selection metrics are not an untouched test-set accuracy claim.
...
| `POST /agent/run` | Procurement tool orchestration, or explicitly labeled local fact lookup without OpenAI |

Dedicated server configuration in `intelligence/.env` (UTF-8):

- `OPENAI_API_KEY`: optional fresh key, read only from this dedicated file on each request. Root PHP `.env` and inherited process credentials are ignored. Never included in responses or logs. `.env.example` contains a blank placeholder; `.env` is gitignored.
...
All deterministic endpoints and local analysis work without a key. Health's `agent_available` indicates that analysis is available, while `metadata.agent_mode` is `local` or `openai`. `metadata.openai_configured` indicates configuration, not verified provider access. Missing artifacts produce a degraded health response and 503 on data endpoints.
...
The OpenAI integration follows the official [Responses API function-calling flow](https://developers.openai.com/api/docs/guides/function-calling): send strict function schemas, execute returned calls on the server, return outputs associated with each `call_id`, and preserve response items across rounds. Requests use `store=False`, a fixed official API base URL, six-second provider timeouts without retries, at most three rounds and twelve executed/attempted calls. With a configured key, user requests and selected tool results are transmitted to OpenAI; no environment variables or credentials are included in tool results.
...
The actual `result` contains the complete authoritative tool payload. Unknown tools/extra arguments are rejected. Provider errors are sanitized. Missing keys and provider failures use HTTP 200 with `data.mode="local"`, `data.status="local_analysis"`, and an explicit reason (`key_not_configured` or `provider_unavailable`). Local mode uses bounded keyword/SKU/article lookup, not an LLM. The UI labels it as local; the Russian answer is always rendered from facts. Lists show eight rows in prose while structured data retains the complete result.
...
The suite covers algorithms, API, null handling, local/provider fallback, bounded tools and adversarial model outputs. It also checks seasonal peaks with sustained growth, stockout correction reaching reorder quantities, and per-SKU selection. `data/processed/api_demo.json` is an older demonstration artifact, not the current API state. No live paid model call was performed during this validation. Raw workbook hashes were verified unchanged. Jetson/RealSense remains outside this integration. See `docs/HACKATHON-READINESS.md` for acceptance gaps.
