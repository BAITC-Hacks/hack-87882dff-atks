...
from .features import build_features, forecast_source, eligibility, FEATURES
from .baseline import BASELINES, predict_baseline, nonnegative, evaluate, metrics, select_models
...
def main():
    ...
    source = forecast_source(read('corrected_demand'))
    ...
    per_sku = select_models(validation, selected)
    methods = future_features.sku.map(per_sku).fillna(selected)
    forecast_values = np.zeros(len(future_features))
    fallback = np.zeros(len(future_features), dtype=bool)
    effective = methods.to_numpy().copy()
    for method in methods.unique():
        mask = methods.eq(method).to_numpy()
        rows = future_features.loc[mask]
        if method == 'catboost':
            forecast_values[mask] = nonnegative(final_model.predict(rows[FEATURES]))
        else:
            prediction, missing = predict_baseline(rows, method)
            forecast_values[mask], fallback[mask] = prediction, missing
            effective[mask] = np.where(missing, np.where(rows.rolling_mean_3.notna(),
                                       'rolling_3_fallback', 'historical_mean_fallback'), method)
    forecast = pd.DataFrame({'sku': future_features.sku.values, 'forecast_date': forecast_date,
                             'forecast_demand': forecast_values, 'model_name': methods.to_numpy(),
    ...
    forecast['effective_model_name'] = effective
    forecast['annual_growth_factor'] = future_features.annual_growth_factor.to_numpy()
    forecast['seasonal_reference_demand'] = future_features.lag_12.to_numpy()
    historical = source[(source.date <= last_complete_month) & source.estimated_lost_demand.notna()]
    lost = historical.groupby('sku').estimated_lost_demand.sum(min_count=1)
    forecast['historical_lost_demand'] = forecast.sku.map(lost)
    forecast['demand_basis'] = 'stockout_corrected_regular_demand'
    ...
    schema = {'features': FEATURES, 'categorical_features': ['sku'], 'target': 'stockout_corrected_regular_demand',
              ...
              'selected_model': 'per_sku_backtest', 'global_fallback_model': selected,
              'seasonal_trend': 'lag_12 * median(last six prior year-on-year ratios); min 3 ratios, bounded [0.5, 2.0]',
              'baseline_fallback': 'native predictor, then past rolling_mean_3, then past history_mean',
    ...
    report = {'metrics': scores, 'native_baseline_metrics': native_metrics,
              'common_native_support_metrics': common_native_metrics, 'folds': folds, 'selected_model': 'per_sku_backtest',
              'global_fallback_model': selected, 'model_selection': 'Per-SKU minimum MAE on at least four validation months; otherwise global WAPE winner.',
              'demand_basis': 'stockout_corrected_regular_demand', 'best_baseline': best_baseline,
              ...
              'limitations': ['Six expanding-window one-month backtests; model selection and metrics share these folds, no untouched final test set.',
                              ...
                              'Per-SKU selection can overfit six validation months; a separate future test period is still required.',
                              'External category/growth forecasts are not integrated because their dated semantics have not been agreed.',
