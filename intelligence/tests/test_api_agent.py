...
class IntegrationTests(unittest.TestCase):
    ...
    def test_health_metadata(self):
        ...
        self.assertEqual(response.json()['metadata']['replenishment_count'], 23)
    ...
    def test_recommendations_list(self):
        ...
        self.assertEqual(positive['total'], 23)
        self.assertEqual(sum(row['recommended_quantity'] for row in positive['items']), 2160)
    ...
    def test_agent_unavailable_without_key(self):
        with patch.dict(os.environ, {'OPENAI_API_KEY': ''}):
            ...
            self.assertEqual(response.json()['data']['status'], 'agent_unavailable')
    ...
    def test_missing_inputs_explicit_in_agent_answer(self):
        ...
        self.assertIn('missing_or_invalid_in_transit', result['answer'])
    ...
    def test_recalculate_uses_existing_engine(self):
        ...
        self.assertEqual(sum(row['recommended_quantity'] or 0 for row in all_rows['items']), 2160)
    ...
    def test_provider_errors_do_not_expose_credentials(self):
        ...
        self.assertEqual(response['data']['status'], 'provider_unavailable')
