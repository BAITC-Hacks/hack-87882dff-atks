...
from intelligence.catalog import build_catalog
...
class Snapshot:
    ...
    def catalog(self):
        try:
            return build_catalog(self.frames, self.directory)
        except (OSError, ValueError, KeyError) as exc:
            raise DataUnavailable('Product labels are unavailable or invalid.') from exc
